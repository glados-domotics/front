<template>
  <div
    id="popup-modal"
    tabindex="-1"
    class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 backdrop max-h-full flex"
    aria-modal="true"
        ...
        <div class="p-4 md:p-5 text-center">
          <form
            ...
            <Input
              ...
              :placeholders="['Nom de l\'appareil']"
            ...
            <Select
              label="Pièce"
              :modelValue="form.room"
              :choices="[...rooms, { id: null, label: 'Aucune' }]"
              @update:modelValue="form.room = $event"/>
            <p>
              <input
...
export default {
    ...
    entity: {
      handler: function (val) {
        this.form =  {
          name: val?.name ?? "",
          id: val?.id ?? "",
          type: val?.type ?? "sensor",
          status: val?.status ?? "on",
          value: val?.value ?? null,
          room: val?.room?.id ?? null,
        }
      },
      deep: true
    }
  ...
  methods: {
    checkForm: function (e) {
      ...
      if(this.form.id !== "") {
        coreApi.glados.editEntity(this.form.id , payload).then(() => {
        ...
        coreApi.glados.createEntity(payload)
          .then(() => {
            this.showModalToggle()
            this.$emit("update:entity")
          })
