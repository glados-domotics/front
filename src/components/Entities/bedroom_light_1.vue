<template>
  <div>
    <div v-on:click="show_bedroom_light1"> Cliquez ici pour afficher les infos des types "lights"
      <br>
      <br>
      <span
        v-for="bedroom_light1 in $store.state.bedroom_light1"
        :key="bedroom_light1.id">
        id : {{bedroom_light1.id}} <br> nom : {{bedroom_light1.name }} <br> type : {{bedroom_light1.type }} <br> status : {{bedroom_light1.status }} <br> valeur : {{bedroom_light1.value}} <br> crée le : {{bedroom_light1.created_at}}
      </span>
    </div>
...
export default ({
  ...
  methods: {
    show_bedroom_light1(){
      this.$store.dispatch("bedroom_light", "light")
    },
    send_modification(){
    ...
    activatetexttospeech(){
      this.$store.dispatch("texttospeech", this.$store.state.bedroom_light1[0])
    }
  }, 
  data(){
    return {
